...
<div class="workingroom">
    ...
    <div class="addOrEdit" >
        <form action="updateRole" method="post">
            角色名: <input type="text" name="role_name" value="${role.role_name}"> <br>
            ...
            <div style="text-align:left;width:300px;margin:0px auto;padding-left:50px">
                <c:forEach items="${ps}" var="p">
                    ...
                    <c:forEach items="${currentPermissions}" var="currentPermission">
                        <c:if test="${p.resource_id==currentPermission.resource_id}">
                            <c:set var="hasPermission" value="true" />
                        </c:if>
                    </c:forEach>
                    <input type="checkbox"  ${hasPermission?"checked='checked'":"" } name="permissionIds" value="${p.resource_id}"> ${p.resource_name}<br>
                </c:forEach>
            </div>
