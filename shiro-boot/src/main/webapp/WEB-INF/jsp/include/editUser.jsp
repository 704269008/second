...
<div class="workingroom">
    ...
    <div class="addOrEdit" >
        <form action="updateUser" method="post">
            ...
            密码: <input type="password" name="userPassword" value="" placeholder="留空就表示不修改密码"> <br><br>
            配置角色:<br>
            <div style="text-align:left;width:300px;margin:0px auto;padding-left:50px">
                <c:forEach items="${rs}" var="r">
                    ...
                    <c:forEach items="${currentRoles}" var="currentRole">
                        <c:if test="${r.role_id==currentRole.role_id}">
                            <c:set var="hasRole" value="true" />
                        </c:if>
                    </c:forEach>
                    <input type="checkbox"  ${hasRole?"checked='checked'":"" } name="roleIds" value="${r.role_id}"> ${r.role_name}<br>
                </c:forEach>
            </div>
